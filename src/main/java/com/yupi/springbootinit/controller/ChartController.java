...
public class ChartController {
    ...
    public BaseResponse<BiResponse> genChartByAi(@RequestPart("file") MultipartFile multipartFile,
        ...
        long bimodelid=1775115030774558721L;
...
//        3,30
        ...
        //调用AI 得到AI对话数据
        String doChat_result=aiManager.doChat(bimodelid,userInput.toString());
        //拆分数据
        String[] splits=doChat_result.split("【【【【【");
        if(splits.length<3){
...
//        3,30
        ...
        CompletableFuture.runAsync(()->{
            //先修改图表任务状态为“执行中”。等执行成功后，修改为“已完成”，保存执行结果；执行失败后，修改状态为“失败”，记录任务失败信息
            Chart updateChart=new Chart();
            updateChart.setId(chart.getId());
            updateChart.setStatus("running");
            boolean b=chartService.updateById(updateChart);
            if (!b){
                handleChartUpdateError(chart.getId(),"图标执行中状态更改失败");
                return;
            }
            //调用AI 得到AI对话数据
            String doChat_result=aiManager.doChat(bimodelid,userInput.toString());
